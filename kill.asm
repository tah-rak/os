...
main(int argc, char **argv)
{
  ...
  exit();
    ...
    304a:	50                   	push   %eax
    304b:	50                   	push   %eax
    304c:	68 38 37 00 00       	push   $0x3738
    3051:	6a 02                	push   $0x2
    3053:	e8 b8 03 00 00       	call   3410 <printf>
    exit();
    ...
    336c:	89 d1                	mov    %edx,%ecx
{
  ...
  do{
    ...
    3399:	0f b6 92 ac 37 00 00 	movzbl 0x37ac(%edx),%edx
    ...
    3419:	8b 5d 0c             	mov    0xc(%ebp),%ebx
{
  ...
  for(i = 0; fmt[i]; i++){
    ...
    3489:	ff 24 85 54 37 00 00 	jmp    *0x3754(,%eax,4)
        putc(fd, c);
      }
      state = 0;
    }
  ...
  write(fd, &c, 1);
    ...
    3597:	90                   	nop
          s = "(null)";
    3598:	ba 4c 37 00 00       	mov    $0x374c,%edx
        while(*s != 0){
...
free(void *ap)
{
  ...
  for(p = freep; !(bp > p && bp < p->s.ptr); p = p->s.ptr)
    35b1:	a1 64 3a 00 00       	mov    0x3a64,%eax
{
  ...
  freep = p;
    ...
    35f1:	5b                   	pop    %ebx
  freep = p;
    35f2:	89 15 64 3a 00 00    	mov    %edx,0x3a64
}
    35f8:	5e                   	pop    %esi
    35f9:	5f                   	pop    %edi
    35fa:	5d                   	pop    %ebp
  ...
  freep = p;
    362c:	89 15 64 3a 00 00    	mov    %edx,0x3a64
    p->s.size += bp->s.size;
...
malloc(uint nbytes)
{
  ...
  if((prevp = freep) == 0){
    364c:	8b 3d 64 3a 00 00    	mov    0x3a64,%edi
  nunits = (nbytes + sizeof(Header) - 1)/sizeof(Header) + 1;
  ...
  for(p = prevp->s.ptr; ; prevp = p, p = p->s.ptr){
    ...
    if(p == freep)
    3691:	8b 3d 64 3a 00 00    	mov    0x3a64,%edi
  ...
  return freep;
    36bf:	8b 15 64 3a 00 00    	mov    0x3a64,%edx
    ...
    36ec:	89 70 04             	mov    %esi,0x4(%eax)
      freep = prevp;
    36ef:	89 15 64 3a 00 00    	mov    %edx,0x3a64
}
    ...
    36ff:	c3                   	ret    
    base.s.ptr = freep = prevp = &base;
    3700:	c7 05 64 3a 00 00 68 	movl   $0x3a68,0x3a64
    3707:	3a 00 00 
    base.s.size = 0;
    370a:	bf 68 3a 00 00       	mov    $0x3a68,%edi
    base.s.ptr = freep = prevp = &base;
    370f:	c7 05 68 3a 00 00 68 	movl   $0x3a68,0x3a68
    3716:	3a 00 00 
  for(p = prevp->s.ptr; ; prevp = p, p = p->s.ptr){
    3719:	89 fa                	mov    %edi,%edx
    base.s.size = 0;
    371b:	c7 05 6c 3a 00 00 00 	movl   $0x0,0x3a6c
    3722:	00 00 00 
    if(p->s.size >= nunits){
